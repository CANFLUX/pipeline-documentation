## 4.2. &nbsp; Full Documentation: Project Directory Structure and Matlab Configuration

**This section provides the same instructions as the quick-start set up in section 4.1, but in much greater detail.** 

### How to set up your project directory structure and configure Matlab to work with Biomet.net
...
* Now, if you navigate to that `SITEID1` directory, you should see two more folders: `Derived_Variables` and `log` (figure 4.7):
...
* The purpose of these will be explained later in section(s) XXX... <!-- or just add here? -->
...
#### Sites directory
* Raw, *uncleaned* data from your site(s) will be stored in the `<projectPath>/Sites` directory, under the appropriate siteID (this is covered in section 5). The data in this directory should remain *untouched* since we always want to preserve a copy of the raw data. 
...
Once you have everything appearing as described above, you can now move on to the next step which configures Matlab for the directory structure you just created.
...
4. Optional final step: open (but do not run) `get_TAB_project_configuration.m`, located in `/<projectPath>/Matlab/` and enter your projectName (user-preferred name, this is not used for processing). The file should look like this:
...
With that “main” script the entire data processing is documented and it can be easily replicated. Insert as many informative comments as you can - it will help if the process needs to be reviewed later on.
