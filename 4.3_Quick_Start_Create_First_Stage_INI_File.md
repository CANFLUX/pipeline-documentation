...
The following download provides templates for all stages of cleaning to use later (even though this section only covers the first stage).
...
3. The files in the `Tutorial_filled_sample_files` folder are completed, "filled-in" versions of the TEMPLATE files that work with the sample data available for download in section 5 (check section number XXX).
